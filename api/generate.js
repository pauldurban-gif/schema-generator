...
export default async function handler(req, res) {
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: 4000,
      system: 'You are a Schema.org expert. Generate valid, well-formed JSON-LD markup. IMPORTANT: In the schema, use proper PostalAddress objects for addresses (with streetAddress, addressLocality, etc.), not just the organization name. The "location" or "address" field should contain actual address information, never just the organization name.',
      tools: [{
